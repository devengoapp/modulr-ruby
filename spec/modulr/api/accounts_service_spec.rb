...
RSpec.describe Modulr::API::AccountsService, :unit, type: :client do
  ...
  describe "accounts create" do
    ...
    context "when fails" do
      before do
        stub_request(:post, %r{/customers/C0000001/accounts}).to_return(
          read_http_response_fixture("accounts/create", "failure")
        )
      end

      let(:failed_created_account) do
        accounts.create(
          customer_id: "C0000001",
          currency: "EUR",
          product_code: nil,
      ...
      it "returns bad_request response" do
        expect { failed_created_account }.to raise_error(Faraday::BadRequestError)
      end
    end
  end
  ...
  describe "accounts close" do
    ...
    context "when fails" do
      before do
        stub_request(:post, %r{/accounts/unexisting_account_id/close}).to_return(
          read_http_response_fixture("accounts/close", "failure")
        )
      end

      let(:failed_closed_account) do
        accounts.close(account_id: "unexisting_account_id")
      end

      it "returns bad_request response" do
        expect { failed_closed_account }.to raise_error(Faraday::BadRequestError)
      end
    end
  end
end
